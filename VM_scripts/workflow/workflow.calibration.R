...
years = as.numeric(substr(settings$run$start.date, 1, 4)):as.numeric(substr(settings$run$end.date, 1, 4))
...
agb.melt = reshape::melt(agb.array)
colnames(agb.melt) = c('species','year','ensemble', 'agb')
agb.melt$year = agb.melt$year + years[1] - 1
agb.melt$species = plyr::mapvalues(agb.melt$species, from = c(1:nspec), to = sppname)
agb.melt$species = as.factor(agb.melt$species)
...
agb.pft.obs = matrix(0,length(inds),nspec)
agb.pft.var = matrix(0,length(inds),nspec)
agb.obs.tot.var = vector()
track = 1
for (yr in inds){
  agb.pft.obs[track,] = obs.mean[[yr]][2:5]
  agb.pft.var[track,] = diag(obs.cov[[yr]])[2:5]
...
agb.pft.obs.melt$year = agb.pft.obs.melt$year + 1900 - 1
agb.pft.obs.melt$species = plyr::mapvalues(agb.pft.obs.melt$species, from = c(1:nspec), to = sppname)
...
agb.pft.var.melt$species = plyr::mapvalues(agb.pft.var.melt$species, from = c(1:nspec), to = sppname)
agb.pft.var.melt$year = agb.pft.var.melt$year + 1900 - 1
...
pl1 = ggplot() +
  geom_line(data = agb.pft.obs.melt, aes(x = year, y = b05.data), linetype = 'dashed') +
  geom_line(data = agb.pft.obs.melt, aes(x = year, y = b95.data), linetype = 'dashed') +
  geom_line(data = agb.pft.obs.melt, aes(x = year, y = agb.obs),size = 1.2) +
  geom_line(data = agb.melt, aes(x = year, y = agb, group = ensemble), color = 'red') +
  facet_grid(species~., scales='free') +
  labs(col = 'species', y = 'aboveground biomass (Kg C / m2)') 
#pl1
ggsave(plot = pl1, file.path(plot.dir,'agb.plot.jpg'))
...
tree.melt$species = plyr::mapvalues(tree.melt$species, from = c(1:nspec), to = sppname)
...
lgf.melt$species = plyr::mapvalues(lgf.melt$species, from = c(1:nspec), to = sppname)
tree_num = lgf.melt %>%
